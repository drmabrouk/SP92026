...
class EESS_Student_Data_Service {
    ...
    public static function normalize_grade($input) {
        ...
        if ($num >= 1 && $num <= 12) {
            $grade_map = array(
                1 => 'Training Group ', 2 => 'Training Group ', 3 => 'Training Group ', 4 => 'Training Group ',
                5 => 'Training Group ', 6 => 'Training Group ', 7 => 'Training Group ', 8 => 'Training Group ',
                9 => 'Training Group ', 10 => 'Training Group ', 11 => 'Training Group  ', 12 => 'Training Group  '
            );
            return $grade_map[$num];
        }
        return !empty($input) ? sanitize_text_field($input) : 'Training Group ';
    }
    ...
    public static function normalize_allergies($input) {
        ...
        foreach ($items as $item) {
            if (!empty($item) && $item !== 'No  ' && $item !== 'No Known Allergy') {
                $cleaned[] = sanitize_text_field($item);
            }
        }
        return !empty($cleaned) ? implode('; ', array_unique($cleaned)) : 'No  ';
    }
    ...
    public static function process_and_save_student($data) {
        ...
        if (empty($raw_name)) {
            return new WP_Error('missing_required_name', 'Player Name  .');
        }
        if (empty($raw_school)) {
            return new WP_Error('missing_required_school', '/ Academy   .');
        }
        if (empty($raw_grade)) {
            return new WP_Error('missing_required_grade', 'Training Group   .');
        }
        if (empty($raw_sec)) {
            return new WP_Error('missing_required_section', 'Training Group / Training Group  .');
        }
        ...
        if ($curr_user_id > 0) {
            ...
            $can_edit_student = current_user_can('manage_options') || current_user_can('_No') || in_array('administrator', $user_roles) || in_array('sm_system_admin', $user_roles) || in_array('sm_principal', $user_roles) || in_array('sm_supervisor', $user_roles) || in_array('sm_discipline_supervisor', $user_roles);

            if (!$can_edit_student) {
                return new WP_Error('unauthorized', ' No  No  Add  Edit  No.');
            }
                    ...
                    if ($student_id > 0) {
                        ...
                        if ($existing_stu) {
                            ...
                            if ($st_inst > 0 && !empty($allowed_insts) && !in_array($st_inst, $allowed_insts, true)) {
                                return new WP_Error('access_denied', ' No  No Edit No   .');
                            }
                        }
                    }
                }
        ...
        if (empty($name)) {
            return new WP_Error('missing_name', 'Player Name  .');
        }
        ...
        if (!empty($national_id)) {
            ...
            if ($existing_nat_stu) {
                return new WP_Error('duplicate_national_id', ' National ID  —    .');
            }
        ...
        $fields = array(
            ...
            'guardian_relationship' => sanitize_text_field($data['guardian_relationship'] ?? ''),
            ...
            'emirate'               => self::normalize_emirate($data['emirate'] ?? ''),
        ...
        if ($student_id > 0) {
            ...
            if ($updated === false) {
                return new WP_Error('db_update_failed', ' Update  No   : ' . $wpdb->last_error);
            }
            $final_id = $student_id;
            ...
            if ($institution_id > 0) {
                ...
                if (intval($saved_inst_id) !== intval($institution_id)) {
                    return new WP_Error('db_verification_failed', '   Save Organization    .    Save .');
                }
            }
        } else {
            ...
            if ($inserted === false || !$wpdb->insert_id) {
                return new WP_Error('db_insert_failed', ' Add No   : ' . $wpdb->last_error);
            }

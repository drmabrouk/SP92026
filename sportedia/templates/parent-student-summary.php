...
<div style="background: #fff; border: 1px solid var(--sm-border-color); border-radius: 16px; overflow: hidden; margin-bottom: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
    <div style="background: linear-gradient(135deg, var(--sm-primary-color) 0%, #2c3e50 100%); padding: 40px; display: flex; gap: 30px; align-items: center; color: white;">
        ...
        <div style="flex: 1;">
            <h2 style="margin: 0 0 12px 0; border: none; padding: 0; color: white; font-size: 2em; font-weight: 800;">No: <?php echo esc_html($student->name); ?></h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; font-size: 0.95em; opacity: 0.9;">
                ...
                <span style="display: flex; align-items: center; gap: 8px;"><span class="dashicons dashicons-id"></span>  No: <?php echo esc_html($student->student_code); ?></span>
                <span style="display: flex; align-items: center; gap: 8px;"><span class="dashicons dashicons-admin-site"></span> Gender: <?php echo esc_html($student->nationality ?: ' '); ?></span>
        ...
        <div style="text-align: left; display: flex; flex-direction: column; gap: 10px; align-items: flex-end;">
            ...
            <?php endif; ?>
            <a href="<?php echo admin_url('admin-ajax.php?action=sm_print&print_type=disciplinary_report&student_id=' . $student->id); ?>" target="_blank" class="sm-btn" style="background: white !important; color: var(--sm-primary-color) !important; width: auto; font-size: 14px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"> Upload   PDF</a>
        </div>
    </div>
</div>
...
<div class="sm-card-grid" style="margin-bottom: 40px;">
    <div class="sm-stat-card" style="border-right: 5px solid #F63049;">
        <div style="font-size: 0.85em; color: var(--sm-text-gray); margin-bottom: 10px; font-weight: 700;"> </div>
    ...
    <div class="sm-stat-card">
        ...
        <div style="font-size: 1.2em; font-weight: 700; color: var(--sm-secondary-color); margin-top: 15px;">
            ...
            echo isset($types[$stats['frequent_type']]) ? $types[$stats['frequent_type']] : 'No ';
            ?>
        </div>
    </div>
    <div class="sm-stat-card">
        <div style="font-size: 0.85em; color: var(--sm-text-gray); margin-bottom: 10px; font-weight: 700;">  </div>
        <div style="font-size: 1.1em; font-weight: 700; color: #27ae60; margin-top: 15px;"><?php echo $stats['last_action'] ?: 'No '; ?></div>
    </div>
</div>

<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 40px;">
    <div style="background: #fff; padding: 30px; border-radius: 12px; border: 1px solid var(--sm-border-color);">
        <h3 style="margin-top:0; border-bottom: 2px solid var(--sm-primary-color); padding-bottom: 10px;"> </h3>
        <?php if (empty($student_assignments)): ?>
            <p style="padding: 20px; text-align: center; color: var(--sm-text-gray);">No   .</p>
        <?php else: ?>
                ...
                <?php foreach ($student_assignments as $assign): ?>
                    <div style="padding: 15px; border: 1px solid #eee; border-radius: 8px; margin-bottom: 10px;">
                        ...
                        <?php if ($assign->file_url): ?>
                            <a href="<?php echo esc_url($assign->file_url); ?>" target="_blank" class="sm-btn" style="height: 28px; font-size: 10px; margin-top: 10px; width: auto;"> Upload </a>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <div style="background: #fff; padding: 30px; border-radius: 12px; border: 1px solid var(--sm-border-color);">
        <h3 style="margin-top:0; border-bottom: 2px solid var(--sm-accent-color); padding-bottom: 10px;"> No No</h3>
        <?php if (!$supervisor): ?>
            <p style="padding: 20px; text-align: center; color: var(--sm-text-gray);">     Training Group .</p>
        <?php else: ?>
            <div style="background: #f8fafc; padding: 15px; border-radius: 8px; margin-bottom: 20px; display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <div style="font-weight: 800; font-size: 0.9em;">: <?php echo esc_html($supervisor->display_name); ?></div>
                    <div style="font-size: 11px; color: #38a169;"> NoNo </div>
                </div>
            </div>
            <div class="sm-form-group">
                <label class="sm-label"> No:</label>
                <textarea id="student-inquiry-msg" class="sm-textarea" rows="4" placeholder="    Send  ..."></textarea>
            </div>
            <button onclick="sendStudentInquiry(<?php echo $supervisor->ID; ?>)" class="sm-btn" style="background: var(--sm-accent-color);">Send No </button>
        <?php endif; ?>
    </div>
</div>
...
<div style="background: #fff; padding: 30px; border-radius: 12px; border: 1px solid var(--sm-border-color); margin-bottom: 30px; grid-column: span 2;">
    <h3 style="margin-top:0; border-bottom: 2px solid var(--sm-secondary-color); padding-bottom: 10px;">Coaches  Training Group</h3>
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(230px, 1fr)); gap: 15px;">
        ...
        if (empty($assigned_teachers)):
            echo '<p style="grid-column: 1/-1; text-align:center; color:#718096;">No    .</p>';
        else:
        ...
        <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 12px;">
            ...
            <div>
                ...
                <div style="font-size: 11px; color: var(--sm-primary-color); font-weight: 600;"><?php echo esc_html($spec ?: ''); ?></div>
            </div>
        </div>
    ...
    <div style="background: #fff; padding: 30px; border-radius: 12px; border: 1px solid var(--sm-border-color); grid-column: span 2; margin-bottom: 30px;">
        <h3 style="margin-top:0; border-bottom: 2px solid #3498db; padding-bottom: 10px;"> Academy</h3>
        <div id="student-grades-display">
            <div style="text-align: center; padding: 20px; color: var(--sm-text-gray);"> Upload ...</div>
        </div>
    </div>
        ...
        .then(res => {
            if (res.success) {
                if (res.data.length === 0) {
                    container.innerHTML = '<p style="text-align:center; padding:20px; color:#718096;">No    .</p>';
                } else {
                    let html = '<table class="sm-table" style="box-shadow:none; border:none;"><thead><tr><th>Sport Activity</th><th>Training Group</th><th></th></tr></thead><tbody>';
                    res.data.forEach(g => {

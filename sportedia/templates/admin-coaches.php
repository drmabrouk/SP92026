...
<div class="sm-content-wrapper" dir="rtl">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h3 style="margin:0; border:none; padding:0;">  </h3>
        <?php if (current_user_can('_Users')): ?>
            <div style="display:flex; gap:10px;">
                <button onclick="executeBulkDeleteUsers()" class="sm-btn" style="width:auto; background:#e53e3e;">Delete Users </button>
                <button onclick="document.getElementById('teacher-csv-import-form').style.display='block'" class="sm-btn" style="width:auto; background:var(--sm-secondary-color);">Import  (CSV)</button>
                <button onclick="document.getElementById('add-teacher-modal').style.display='flex'" class="sm-btn" style="width:auto;">+ Add  </button>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div id="teacher-csv-import-form" style="display:none; background: #f8fafc; padding: 30px; border: 2px dashed #cbd5e0; border-radius: 12px; margin-bottom: 30px;">
        <h3 style="margin-top:0; color:var(--sm-secondary-color);"> Import  (CSV)</h3>

        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #e2e8f0; margin-bottom:20px;">
            <p style="font-size:13px; font-weight:700; margin-bottom:10px;">   :</p>
            <table style="width:100%; font-size:11px; border-collapse:collapse; text-align:center;">
                <thead>
                    <tr style="background:#edf2f7;">
                        ...
                        <th style="border:1px solid #cbd5e0; padding:5px;"></th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="border:1px solid #cbd5e0; padding:5px;"> </td>
        ...
        <form method="post" enctype="multipart/form-data" onsubmit="return handleImportSubmit(this, 'sm_import_teachers_csv')">
            ...
            <div class="sm-form-group">
                <label class="sm-label">  CSV :</label>
            ...
            <div style="display:flex; gap:10px; margin-top:20px;">
                <button type="submit" name="sm_import_teachers_csv" class="sm-btn" style="width:auto; background:#27ae60;">Import  </button>
    ...
    <div class="sm-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee;">
        <a href="<?php echo remove_query_arg('role_filter'); ?>" class="sm-tab-btn <?php echo empty($_GET['role_filter']) ? 'sm-active' : ''; ?>" style="text-decoration:none;"></a>
        ...
        <a href="<?php echo add_query_arg('role_filter', 'sm_coordinator'); ?>" class="sm-tab-btn <?php echo ($_GET['role_filter'] ?? '') == 'sm_coordinator' ? 'sm-active' : ''; ?>" style="text-decoration:none;"></a>
        <a href="<?php echo add_query_arg('role_filter', 'sm_supervisor'); ?>" class="sm-tab-btn <?php echo ($_GET['role_filter'] ?? '') == 'sm_supervisor' ? 'sm-active' : ''; ?>" style="text-decoration:none;"></a>
        <a href="<?php echo add_query_arg('role_filter', 'sm_principal'); ?>" class="sm-tab-btn <?php echo ($_GET['role_filter'] ?? '') == 'sm_principal' ? 'sm-active' : ''; ?>" style="text-decoration:none;"> Academy </a>
        <a href="<?php echo add_query_arg('role_filter', 'sm_system_admin'); ?>" class="sm-tab-btn <?php echo ($_GET['role_filter'] ?? '') == 'sm_system_admin' ? 'sm-active' : ''; ?>" style="text-decoration:none;">  </a>
    </div>
    ...
    <div style="background: white; padding: 30px; border: 1px solid var(--sm-border-color); border-radius: var(--sm-radius); margin-bottom: 30px; box-shadow: var(--sm-shadow);">
        <form method="get" style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 20px; align-items: end;">
            ...
            <div class="sm-form-group" style="margin-bottom:0;">
                <label class="sm-label">Search   (//):</label>
                <input type="text" name="teacher_search" class="sm-input" value="<?php echo esc_attr(isset($_GET['teacher_search']) ? $_GET['teacher_search'] : ''); ?>" placeholder="  Search...">
            </div>
            ...
            <div class="sm-form-group" style="margin-bottom:0;">
                ...
                <select name="role_filter" class="sm-select">
                    ...
                    <option value="sm_system_admin" <?php selected($_GET['role_filter'] ?? '', 'sm_system_admin'); ?>> </option>
                    <option value="sm_principal" <?php selected($_GET['role_filter'] ?? '', 'sm_principal'); ?>> Academy </option>
                    <option value="sm_supervisor" <?php selected($_GET['role_filter'] ?? '', 'sm_supervisor'); ?>></option>
                    <option value="sm_coordinator" <?php selected($_GET['role_filter'] ?? '', 'sm_coordinator'); ?>> </option>
                    <option value="sm_teacher" <?php selected($_GET['role_filter'] ?? '', 'sm_teacher'); ?>></option>
                </select>
            </div>
    ...
    <div class="sm-table-container">
        <table class="sm-table">
            ...
            <tbody>
                ...
                if (empty($users)): ?>
                    <tr><td colspan="6" style="padding: 40px; text-align: center;">No    Search.</td></tr>
                <?php else: ?>
    ...
    <div id="edit-teacher-modal" class="sm-modal-overlay">
        <div class="sm-modal-content">
            ...
            <form id="edit-teacher-form">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                    ...
                    <div class="sm-form-group">
                        ...
                        <select name="role" id="edit_t_role" class="sm-select" onchange="toggleAssignFields(this, 'edit')">
                            <option value="sm_system_admin"> </option>
                    ...
                    <div class="sm-form-group">
                        ...
                        <select name="account_status" id="edit_t_status" class="sm-select">
                            ...
                            <option value="restricted"> (No  )</option>
                        </select>
                    </div>
                    <div class="sm-form-group">
                        <label class="sm-label">   ():</label>
                ...
                <button type="submit" class="sm-btn" style="margin-top:20px;">Save </button>
            </form>
        </div>
    </div>
            ...
            <form id="add-teacher-form">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                    ...
                    <div class="sm-form-group" style="grid-column: span 2;">
                        <label class="sm-label">Password (    10 ):</label>
                        <input type="password" name="user_pass" class="sm-input" placeholder="********">
                    </div>
                </div>
                <button type="submit" class="sm-btn" style="margin-top:20px;">  </button>
            </form>
        </div>
    </div>
    ...
    function executeBulkDeleteUsers() {
        ...
        .then(res => {
            if (res.success) {
                smShowNotification(' Delete Users ');

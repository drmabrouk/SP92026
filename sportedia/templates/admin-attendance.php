...
<div class="sm-attendance-page" dir="rtl" style="font-family: 'Cairo', sans-serif !important;">
        ...
        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: nowrap;">
            ...
            <div style="position: relative; display: inline-block;">
                ...
                <div id="eess-absence-dropdown" style="display: none; position: absolute; left: 0; top: 115%; background: #ffffff; border: 1px solid #cbd5e1; border-radius: 14px; width: 220px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 99999; padding: 6px 0; text-align: right;">
                    <a href="javascript:void(0)" onclick="printAbsenceReport('daily'); document.getElementById('eess-absence-dropdown').style.display='none';" style="display: block; padding: 10px 16px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9; font-weight: 700;">  </a>
                    <a href="javascript:void(0)" onclick="printAbsenceReport('term'); document.getElementById('eess-absence-dropdown').style.display='none';" style="display: block; padding: 10px 16px; color: #334155; font-size: 12px; text-decoration: none; font-weight: 700;">   (Training Group)</a>
                </div>
            </div>

            <div style="display: flex; align-items: center; gap: 6px;">
                <label for="attendance-filter-date" style="font-size: 12px; font-weight: 700; color: #475569; white-space: nowrap;">:</label>
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 40px;">
        ...
        <div class="sm-stat-card" style="padding: 15px; background: #f8fafc; border: 1px solid #e2e8f0;">
            <div style="font-size: 11px; color: var(--sm-text-gray); font-weight: 700;"> No</div>
        ...
        <div class="sm-stat-card" style="padding: 15px; border: 1px solid #c6f6d5; background: #f0fff4;">
            <div style="font-size: 11px; color: #2f855a; font-weight: 700;"></div>
        ...
        <div class="sm-stat-card" style="padding: 15px; border: 1px solid #fed7d7; background: #fff5f5;">
            <div style="font-size: 11px; color: #c53030; font-weight: 700;"></div>
        ...
        <div class="sm-stat-card" style="padding: 15px; border: 1px solid #feebc8; background: #fffaf0;">
            <div style="font-size: 11px; color: #c05621; font-weight: 700;"></div>
    ...
    <div style="background: #fff; padding: 20px; border-radius: 12px; border: 1px solid var(--sm-border-color); margin-bottom: 30px; display: flex; gap: 15px;">
        <input type="text" id="card-search" class="sm-input" placeholder="Search    ..." onkeyup="filterAttendanceCards()">
    ...
    <div id="attendance-cards-grid">
        ...
        <div class="sm-attendance-card"
            ...
            <div style="flex: 2; display: flex; align-items: center; gap: 25px; cursor: pointer;" onclick="openAttendanceModal('<?php echo esc_js($card['class_name']); ?>', '<?php echo esc_js($card['section']); ?>')">
                ...
                <div>
                    <h4 style="margin: 0; font-weight: 900; color: var(--sm-dark-color); font-size: 1.1em;"><?php echo esc_html($card['class_name']); ?> -  <?php echo esc_html($card['section']); ?></h4>
                    <div style="font-size: 12px; color: var(--sm-text-gray); font-weight: 700; margin-top: 4px;">
                         No: <?php echo $card['student_count']; ?>
                    </div>
                </div>
            </div>
            ...
            <div style="flex: 1; display: flex; flex-direction: column; gap: 5px;">
                ...
                <div style="display: flex; gap: 8px; font-size: 11px; font-weight: 700;">
                    <span style="color: #38a169;">: <?php echo $card['stats']['present']; ?></span>
                    <span style="color: #e53e3e;">: <?php echo $card['stats']['absent']; ?></span>
                    <span style="color: #ecc94b;">: <?php echo $card['stats']['late']; ?></span>
                </div>
            </div>

            <div style="flex: 1; display: flex; align-items: center; gap: 15px; border-right: 1px dashed #e2e8f0; padding-right: 15px;">
                <div>
                    <div style="font-size: 10px; color: var(--sm-text-gray); font-weight: 700; margin-bottom: 2px;"> Mother:</div>
                ...
                <button onclick="resetClassCode('<?php echo esc_js($card['class_name']); ?>', '<?php echo esc_js($card['section']); ?>', this)" class="sm-btn sm-btn-outline" style="padding: 4px 8px; font-size: 10px; height: auto;" title="  ">
                    <span class="dashicons dashicons-randomize"></span>
                </button>
            </div>

            <div style="display: flex; gap: 10px;">
                <button onclick="printAttendance('section', '<?php echo esc_js($card['class_name']); ?>', '<?php echo esc_js($card['section']); ?>')" class="sm-btn sm-btn-outline" style="width: 38px; height: 38px; padding: 0;" title="Print ">
...
<div id="sm-attendance-marking-modal" class="sm-modal-overlay">
    <div class="sm-modal-content" style="max-width: 700px;">
        <div class="sm-modal-header">
            <div>
                <h3 id="modal-attendance-title" style="margin: 0;"> </h3>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; background: #f8fafc; padding: 10px; border-radius: 8px;">
            ...
            <div style="display: flex; gap: 8px;">
                <button onclick="setAllAttendance('present')" class="sm-btn" style="background: #38a169; font-size: 11px; padding: 5px 12px;"> </button>
                <button onclick="setAllAttendance('absent')" class="sm-btn" style="background: #e53e3e; font-size: 11px; padding: 5px 12px;"> </button>
            </div>
        </div>

        <div id="attendance-students-list" style="max-height: 400px; overflow-y: auto;">
            <!-- Loaded via AJAX -->
            <div style="text-align: center; padding: 40px; color: var(--sm-text-gray);"> Upload  No...</div>
        </div>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid var(--sm-border-color); display: flex; justify-content: flex-end;">
            <button onclick="closeAttendanceModal()" class="sm-btn" style="background: var(--sm-dark-color);">Close Save</button>
        </div>
    </div>
</div>
...
function openAttendanceModal(className, section) {
    ...
    document.getElementById('modal-attendance-title').innerText = ' : ' + className;
    document.getElementById('modal-attendance-subtitle').innerText = 'Training Group: ' + section + ' | : ' + date;
...
function loadAttendanceStudents(className, section, date) {
    ...
    listContainer.innerHTML = '<div style="text-align: center; padding: 40px; color: var(--sm-text-gray);"> Upload...</div>';
...
function renderStudentsList(students) {
    ...
    if (students.length === 0) {
        listContainer.innerHTML = '<div style="padding: 20px; text-align: center;">No  No   Training Group.</div>';
        return;
    }
            ...
            <tr data-student-id="${s.id}">
                ...
                <td style="text-align: left;">
                    <div class="attendance-options" style="display: flex; gap: 5px; justify-content: flex-end;">
                        <button onclick="saveAttendance(${s.id}, 'present', this)" class="attendance-btn ${s.status === 'present' ? 'active' : ''}" data-status="present" title=""></button>
                        <button onclick="saveAttendance(${s.id}, 'absent', this)" class="attendance-btn ${s.status === 'absent' ? 'active' : ''}" data-status="absent" title=""></button>
                        <button onclick="saveAttendance(${s.id}, 'late', this)" class="attendance-btn ${s.status === 'late' ? 'active' : ''}" data-status="late" title=""></button>
                        <button onclick="saveAttendance(${s.id}, 'excused', this)" class="attendance-btn ${s.status === 'excused' ? 'active' : ''}" data-status="excused" title=""></button>
                    </div>
                </td>
            </tr>
        `;
...
function resetClassCode(grade, section, btn) {
    if (!confirm('       Mother  Training Group')) return;

    ...
    <div style="background: #ffffff; border-radius: 20px; max-width: 780px; width: 100%; border: 1px solid #cbd5e1; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.3); overflow: hidden; display: flex; flex-direction: column; max-height: 92vh;">
        ...
        <div style="background: #0f172a; color: #ffffff; padding: 16px 24px; border-bottom: 1px solid #1e293b; display: flex; justify-content: space-between; align-items: center; width: 100%; box-sizing: border-box;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 800; color: #ffffff; font-family: 'Cairo', sans-serif;">تقديم مخالفة سلوكية لNoعب – إحالة إلى مشرف السلوك</h3>
                    <p style="margin: 3px 0 0 0; font-size: 11.5px; color: #94a3b8; font-weight: 600;">رصد وإحالة المواقف السلوكية بدقة لمتابعة مشرف اNoنضباط</p>
                </div>
            </div>
        ...
        <div style="background: #f8fafc; padding: 12px 24px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
            <div id="ref-step-indicator-1" class="ref-step-node ref-step-active">1. الNoعب</div>
            ...
            <div id="ref-step-indicator-4" class="ref-step-node">4. الأدلة والصNoت</div>
        ...
        <form id="eess-teacher-referral-form" style="padding: 24px; overflow-y: auto; flex: 1;" onsubmit="eessSubmitTeacherReferral(event)">
            ...
            <div id="ref-step-1" class="ref-step-content" style="display: block;">
                <h4 style="margin: 0 0 15px 0; font-size: 14px; font-weight: 800; color: #0f172a;">الخطوة 1: اختيار الNoعب المعني بالإحالة السلوكية</h4>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-size: 12.5px; font-weight: 700; color: #334155; margin-bottom: 6px;">الSearch عن Player Name أو الكود *</label>
                    <input type="text" id="ref_student_query" onkeyup="eessSearchReferralStudent(this.value)" class="sm-input" placeholder="اكتب Player Name أو الكود لمطابقة البيانات..." style="width: 100%; height: 42px; border-radius: 10px; padding: 0 14px; font-size: 13px;">
            ...
            <div id="ref-step-2" class="ref-step-content" style="display: none;">
                <h4 style="margin: 0 0 15px 0; font-size: 14px; font-weight: 800; color: #0f172a;">الخطوة 2: تحديد بند وم عنوان السلوك المNoحظ</h4>
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" id="ref_title" required placeholder="مثال: عدم اNoلتزام بالهدوء والتأخر التكراري عن الحصة..." style="width: 100%; height: 42px; border: 1px solid #cbd5e1; border-radius: 10px; padding: 0 14px; font-size: 13px;">
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        <label style="display: block; font-size: 12.5px; font-weight: 700; color: #334155; margin-bottom: 6px;">تصنيف الموقف المNoحظ</label>
                        <select id="ref_classification" style="width: 100%; height: 42px; border: 1px solid #cbd5e1; border-radius: 10px; padding: 0 12px; font-size: 13px;">
                            ...
                            <option value="labs">في المختبرات / غرف الأActiveة</option>
                            <option value="bus">في الحافلة المدرسية</option>
                            <option value="general">عام / ممر Academy الرياضية</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="ref_degree" style="width: 100%; height: 42px; border: 1px solid #cbd5e1; border-radius: 10px; padding: 0 12px; font-size: 13px;">
                            <option value="1">Level أول (بسيطة)</option>
                            <option value="2">Level ثاني (Intermediateة)</option>
                            <option value="3">Level ثالث (جسيمة)</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div id="ref-step-3" class="ref-step-content" style="display: none;">
                ...
                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-size: 12.5px; font-weight: 700; color: #334155; margin-bottom: 6px;">شرح وتفاصيل الموقف المNoحظ *</label>
                    <textarea id="ref_details" required rows="5" placeholder="اكتب بالتفصيل زمن ووقائع السلوك المNoحظ وتأثيره على البيئة Training Groupية..." style="width: 100%; border: 1px solid #cbd5e1; border-radius: 10px; padding: 10px 14px; font-size: 13px; font-family: 'Cairo', sans-serif;"></textarea>
                </div>
            </div>
            ...
            <div id="ref-step-6" class="ref-step-content" style="display: none;">
                ...
                <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 15px; margin-bottom: 15px; font-size: 13px;">
                    <div style="margin-bottom: 8px;"><strong>الNoعب:</strong> <span id="rev_student_name">---</span></div>
                    ...
                    <div style="color: #dc2626; font-size: 11.5px; font-weight: 700; margin-top: 10px;">⚠️ سيتم Send هذا التقرير مباشرة لمشرف السلوك واNoعتماد لNoنضباط. لن يتم تسجيل المخالفة رسمياً في سجل الNoعب إNo بعد مراجعتها واعتمادها من المشرف.</div>
                </div>
            </div>
...
function eessTeacherReferralNav(dir) {
    if (dir === 1) {
        if (eessRefCurrentStep === 1 && !document.getElementById('ref_selected_student_id').value) {
            if (typeof smShowNotification === 'function') smShowNotification('يرجى اختيار الNoعب المعني أوNoً', true);
            return;
        }
...
function eessSubmitTeacherReferral(e) {
    ...
    .then(res => {
        ...
        if (res.success) {
            if (typeof smShowNotification === 'function') smShowNotification('تم تقديم المخالفة السلوكية بنجاح وهي قيد المراجعة واNoعتماد من مشرف السلوك');
            eessCloseTeacherReferralModal();
            setTimeout(() => location.reload(), 500);
        } else {
            if (typeof smShowNotification === 'function') smShowNotification('خطأ: ' + (res.data || 'تعذر Save البيانات'), true);
        }
    });
}
</script>

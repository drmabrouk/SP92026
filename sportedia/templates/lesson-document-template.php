...
<head>
    <meta charset="UTF-8">
    <title>     - <?php echo esc_html($prep->title); ?></title>
    <style>
        @page {
            size: A4 portrait;
            margin: 10mm 12mm;
...
<body onload="window.print()">
    ...
    <div class="no-print" style="background: #f1f5f9; padding: 10px 16px; border-radius: 10px; margin-bottom: 15px; display: flex; align-items: center; justify-content: space-between; border: 1px solid #cbd5e1;">
        <span style="font-weight: 700; font-size: 12px; color: #334155;">    (A4)  Print  </span>
        <div style="display: flex; gap: 8px; align-items: center;">
            <button onclick="eessShareDocumentPDF()" style="padding: 6px 14px; background: #881337; color: white; border: none; border-radius: 9999px; font-weight: bold; cursor: pointer; font-size: 12px; display: flex; align-items: center; gap: 4px;">  </button>
    ...
    <div class="doc-header">
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                ...
                <p style="margin: 4px 0 0 0; color: #64748b; font-size: 12px; font-weight: 700;">     |  Export: <?php echo current_time('Y-m-d H:i'); ?></p>
            </div>
        </div>
        <div style="text-align: left;">
            ...
            <?php if (!empty($school_phone)): ?>
                <div style="font-size: 11px; color: #64748b; font-family: monospace;">: <?php echo esc_html($school_phone); ?></div>
            <?php else: ?>
        ...
        <tr>
            ...
            <td>
                ...
                if ($prep->status === 'approved') {
                    echo '<span class="status-badge status-approved">✓    </span>';
                } elseif ($prep->status === 'submitted' || $prep->status === 'late') {
                    echo '<span class="status-badge status-submitted">  </span>';
                } else {
                    echo '<span class="status-badge status-draft">  </span>';
                }
                ?>
            </td>
        </tr>
    ...
    <div class="section-card">
        <h3 class="section-card-title">1.    </h3>
        <div class="section-card-body"><?php echo esc_html(!empty($data['objectives']) ? $data['objectives'] : ' '); ?></div>
    </div>

    <?php
    $sub_name = strtolower($prep->subject);
    $is_pe_doc = (strpos($sub_name, '') !== false || strpos($sub_name, '') !== false || strpos($sub_name, 'pe') !== false || strpos($sub_name, 'physical') !== false);
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px;">
        <div class="section-card" style="margin-bottom: 0; border-color: #bae6fd; background: #f0f9ff;">
            <h3 class="section-card-title" style="color: #0369a1; font-size: 11.5px;"> 1.    (Warm-Up)</h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['warmup']) ? $data['warmup'] : ' '); ?></div>
        </div>

        <div class="section-card" style="margin-bottom: 0; border-color: #bbf7d0; background: #f0fdf4;">
            <h3 class="section-card-title" style="color: #15803d; font-size: 11.5px;"> 2.    </h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['physical_prep']) ? $data['physical_prep'] : ' '); ?></div>
        </div>

        <div class="section-card" style="margin-bottom: 0; border-color: #fef3c7; background: #fffbeb;">
            <h3 class="section-card-title" style="color: #b45309; font-size: 11.5px;">⚽ 3.   </h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['skill_prep']) ? $data['skill_prep'] : ' '); ?></div>
        </div>

        <div class="section-card" style="margin-bottom: 0; border-color: #fecdd3; background: #fff1f2;">
            <h3 class="section-card-title" style="color: #991b1b; font-size: 11.5px;"> 4.   No</h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['conclusion']) ? $data['conclusion'] : ' '); ?></div>
        </div>
    </div>
    <?php else: ?>
    <div class="section-card">
        <h3 class="section-card-title">2. <?php echo esc_html($fields['label2']); ?></h3>
        <div class="section-card-body"><?php echo esc_html(!empty($data['warmup']) ? $data['warmup'] : ' '); ?></div>
    </div>

    <div class="section-card">
        <h3 class="section-card-title">3. <?php echo esc_html($fields['label3']); ?></h3>
        <div class="section-card-body"><?php echo esc_html(!empty($data['activities']) ? $data['activities'] : ' '); ?></div>
    </div>
    <?php endif; ?>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px;">
        <div class="section-card" style="margin-bottom: 0; background: #f8fafc;">
            <h3 class="section-card-title" style="color: #0f172a; font-size: 11.5px;">     Country</h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['national_agenda']) ? $data['national_agenda'] : ' '); ?></div>
        </div>

        <div class="section-card" style="margin-bottom: 0; background: #f8fafc;">
            <h3 class="section-card-title" style="color: #0f172a; font-size: 11.5px;">  Sports Activities  </h3>
            <div class="section-card-body" style="font-size: 11px;"><?php echo esc_html(!empty($data['cross_subject']) ? $data['cross_subject'] : ' '); ?></div>
        </div>
    </div>
    ...
    <div class="section-card">
        <h3 class="section-card-title">Notes No  /  No </h3>
        <div class="section-card-body"><?php echo esc_html(!empty($data['notes']) ? $data['notes'] : 'No  Notes  '); ?></div>
    </div>
    ...
    <div style="margin-top: 25px; padding-top: 12px; border-top: 2px solid #cbd5e1; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; text-align: center; page-break-inside: avoid;">
        <div style="text-align: right;">
            <div style="font-weight: 800; font-size: 11.5px; color: #0f172a; margin-bottom: 20px;"> :</div>
        ...
        <div style="text-align: left;">
            <div style="font-weight: 800; font-size: 11.5px; color: #0f172a; margin-bottom: 20px;">  Academy :</div>
            <div style="font-size: 11px; color: #64748b; font-weight: 700;">No : ...........................</div>
        </div>
    </div>

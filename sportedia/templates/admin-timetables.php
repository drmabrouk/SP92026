...
<div class="sm-timetables-container">
    <div style="background: #fff; padding: 25px; border-radius: 12px; border: 1px solid var(--sm-border-color); margin-bottom: 30px; display: flex; justify-content: space-between; align-items: flex-end;">
        ...
        <?php if ($is_admin || $is_principal): ?>
            <button class="sm-btn sm-btn-secondary" onclick="document.getElementById('timetable-settings-modal').style.display='flex'" style="width: auto;">⚙️  </button>
        <?php endif; ?>
    </div>
    ...
    <div style="background: #fff; padding: 25px; border-radius: 12px; border: 1px solid var(--sm-border-color);">
        <h4 style="margin-top:0; color: var(--sm-primary-color);">  : <?php echo $selected_class . ' ' . $selected_section; ?></h4>
        <div class="sm-table-container">
            <table class="sm-table">
                <thead>
                    <tr>
                        <th> / </th>
                        <?php for($i=1; $i<=$periods; $i++) echo "<th> $i</th>"; ?>
                    </tr>
                </thead>
                <tbody>
...
<div id="edit-timetable-modal" class="sm-modal-overlay">
    <div class="sm-modal-content" style="max-width: 450px;">
        ...
        <div class="sm-modal-body">
            ...
            <div class="sm-form-group">
                <label class="sm-label">Sport Activity :</label>
            ...
            <div style="margin: 20px 0; border-top: 1px solid #eee; padding-top: 15px;">
                <label style="font-weight: 800; font-size: 13px; display: block; margin-bottom: 10px; color: #4a5568;">    ():</label>
            ...
            <button class="sm-btn" onclick="smSaveTimetableEntry()">Save Update </button>
        </div>
    </div>
</div>
        ...
        <div class="sm-modal-body">
            ...
            <div class="sm-form-group">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                    <?php
                    $all_days = array('sun' => '', 'mon' => 'No', 'tue' => 'No', 'wed' => '', 'thu' => '', 'fri' => '', 'sat' => '');
            ...
            <button class="sm-btn" onclick="smSaveTimetableSettings()">Save  </button>
        </div>
    </div>
</div>
...
function smEditTimetable(dayKey, period, dayLabel, subjectId, teacherId) {
    ...
    document.getElementById('timetable-info').innerText = dayLabel + ' -  ' + period;
...
function smSaveTimetableEntry() {
    ...
    if (!subjectId || !teacherId) {
        smShowNotification('  Sport Activity ', true);
        return;
    }
    ...
    .then(res => {
        if (res.success) {
            smShowNotification(` Update   (${res.data})  `);
            location.reload();
        } else {

...
(function(window) {
    ...
    window.smSaveProfile = function() {
        ...
        .then(res => {
            if (res.success) {
                smShowNotification(' Update Profile ');
                setTimeout(() => location.reload(), 500);
            } else {
    ...
    window.smInitializeSystem = function() {
        const code = prompt('Confirm       Confirm (1011996):');
        if (!code) return;
    ...
    window.smRefreshSystem = function() {
        ...
        overlay.innerHTML = `
            ...
            <div style="font-weight:700; color:#1e293b; font-size:14px;"> Update   ...</div>
            <style>
        ...
        .then(res => {
            if (res.success) {
                ...
                if (btn) btn.disabled = false;
                smIsRefreshing = false;
                smShowNotification('  Update ', true);
            }
        })
        .catch(err => {
            const overlayEl = document.getElementById('sm-refresh-loading-overlay');
    ...
    <div class="sm-main-header" style="height: 46px; padding: 3px 14px; box-sizing: border-box; display: flex; align-items: center; justify-content: space-between;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div>
                ...
                $my_school_name = get_user_meta($user->ID, 'eess_school_name', true) ?: (get_user_meta($user->ID, 'school_name', true) ?: ($school['school_name'] ?? '   '));
                ?>
                ...
                <div style="display: flex; align-items: center; flex-wrap: wrap; gap: 5px; margin-top: 4px;">
                    ...
                    <?php else: ?>
                        ...
                        <div style="display: inline-block; padding: 2px 10px; background: #fee2e2; color: #991b1b; border-radius: 50px; font-size: 11px; font-weight: 700; border: 1px solid #fca5a5; line-height: 1;">
                            ...
                            echo esc_html($role_labels[$primary_role] ?? ' ');
                            ?>
                        </div>
        ...
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <?php elseif ($is_admin || current_user_can('_')): ?>
            ...
            <button type="button" onclick="eessOpenSupportHelpCapsule()" class="sm-btn" style="background-color: #ffffff !important; color: #dc2626 !important; border: 1.5px solid #ef4444 !important; height: 32px !important; border-radius: 9999px !important; padding: 0 14px !important; font-weight: 800 !important; font-size: 11.5px !important; cursor: pointer !important; display: inline-flex !important; align-items: center !important; gap: 6px !important; box-shadow: 0 2px 6px rgba(239,68,68,0.06) !important; transition: all 0.2s ease !important;">
                ...
                <span style="color: #dc2626 !important; font-weight: 800 !important;"> </span>
            </button>
            ...
            <div class="sm-user-dropdown" style="position: relative;">
                <div class="sm-user-profile-nav" onclick="smToggleUserDropdown()" style="display: flex; align-items: center; gap: 8px; background: white; padding: 4px 10px; border-radius: 50px; border: 1px solid var(--sm-border-color); cursor: pointer;">
                    ...
                    <div style="text-align: right;">
                        ...
                        ?>
                        <div style="font-size: 0.65em; color: #64748b; line-height: 1; font-weight: bold;"><?php echo esc_html($emp_id ? $emp_id : '  '); ?> <span class="dashicons dashicons-arrow-down-alt2" style="font-size: 8px; width: 8px; height: 8px;"></span></div>
                    </div>
                </div>
                <div id="sm-user-dropdown-menu" style="display: none; position: absolute; top: 110%; left: 0; background: white; border: 1px solid var(--sm-border-color); border-radius: 8px; width: 260px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 1000; animation: smFadeIn 0.2s ease-out; padding: 10px 0;">
                    <div id="sm-profile-view">
                        ...
                        <?php endif; ?>
                        <?php if ($is_student || $is_parent): ?>
                            <a href="javascript:smEditProfile()" class="sm-dropdown-item"><span class="dashicons dashicons-lock"></span>  Password</a>
                        <?php endif; ?>
                        <?php if ($is_admin): ?>
                        ...
                        <?php endif; ?>
                        <a href="javascript:location.reload()" class="sm-dropdown-item"><span class="dashicons dashicons-update"></span> Update </a>
                    </div>
                    ...
                    <div id="sm-profile-edit" style="display: none; padding: 15px;">
                        ...
                        <div class="sm-form-group" style="margin-bottom: 10px;">
                            <label class="sm-label" style="font-size: 11px;">No :</label>
    ...
    <div class="sm-admin-layout" style="display: flex; min-height: 800px;">
        ...
        <div class="sm-main-panel" style="flex: 1; min-width: 0; padding: 18px 22px; background: #fff;">
            ...
            <?php if (isset($_GET['sm_admin_msg'])):
                ...
                $messages_map = array(
                    'settings_saved' => ' Save Settings    Update      .',
                    'student_added' => ' Add No  .',
                    'student_deleted' => ' Delete  No   .',
                    'restored' => '   No .',
                    'demo_deleted' => '   Delete   .',
                    'csv_imported' => ' Import    Excel .',
            ...
            $header_map = array(
                ...
                'students' => array(
                    'title' => ' Player Affairs (Student Affairs)',
                    'desc' => '    No  Academy  No  Import Export   .',
                    'button' => ''
                ),
                'teachers' => array(
                    'title' => '  ',
                ...
                'hr-evaluation' => array(
                    'title' => '    ',
                    'desc' => ' No    Annual Training Group    Academy  .',
                    'button' => ''
                ),
                'hr-management' => array(
                    'title' => '  ',
                ...
                'term-plans' => array(
                    'title' => ' Training Group Annual',
                    'desc' => '   Academy     Training Groups No .',
                    'button' => ''
                ),
            ...
            if ($active_tab === 'teachers') {
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    ' . ($is_admin ? '<button onclick="eessOpenUnifiedUserModal(\'add_user\', 0)" class="sm-btn" style="background:#000; border:1px solid #000; color:#fff; border-radius:8px; font-weight:700; height:38px; display:inline-flex; align-items:center; gap:8px; cursor:pointer;"><span class="dashicons dashicons-plus-alt"></span> Add  </button>' : '') . '
                    ...
                    <div style="position: relative; display: inline-block;">
                        ...
                        <div id="eess-user-options-dropdown" style="display: none; position: absolute; left: 0; top: 110%; background: #fff; border: 1px solid #cbd5e1; border-radius: 8px; width: 220px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 99999; padding: 5px 0; text-align: right;">
                            <a href="javascript:void(0)" onclick="document.getElementById(\'user-csv-import-box\').style.display = document.getElementById(\'user-csv-import-box\').style.display === \'none\' ? \'block\' : \'none\'; document.getElementById(\'eess-user-options-dropdown\').style.display=\'none\';" style="display: block; padding: 10px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9; font-weight: 700;"> Import  (CSV)</a>
                            <a href="' . admin_url('admin-ajax.php?action=sm_export_users_csv&nonce=' . wp_create_nonce('eess_admin_action')) . '" onclick="document.getElementById(\'eess-user-options-dropdown\').style.display=\'none\';" style="display: block; padding: 10px 15px; color: #334155; font-size: 12px; text-decoration: none; font-weight: 700;"> Export  (CSV)</a>
                        </div>
                    </div>
                </div>';
            }
            if ($active_tab === 'parents' && ($is_admin || current_user_can('__Mother'))) {
                $header_map['parents']['button'] = '<button onclick="document.getElementById(\'add-parent-modal\').style.display=\'flex\'" class="sm-btn" style="background:#000; border:1px solid #000; color:#fff; border-radius:8px; font-weight:700; height:38px; display:inline-flex; align-items:center; gap:8px; cursor:pointer;"><span class="dashicons dashicons-plus-alt"></span> Add   </button>';
            }
            if ($active_tab === 'grades' && ($is_admin || $is_coordinator || $is_teacher)) {
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <button onclick="document.getElementById(\'eess-grades-import-modal\').style.display=\'flex\'" class="sm-btn sm-btn-outline" style="height:38px; font-size:12px; cursor:pointer; display:inline-flex; align-items:center; gap:5px; border-radius: 8px;"><span class="dashicons dashicons-upload"></span> Import </button>
                </div>';
            }
            if ($active_tab === 'attendance') {
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: nowrap;">
                    <a href="' . home_url('/attendance/') . '" class="sm-btn" style="background:#000; border:1px solid #000; color:#fff !important; border-radius:8px; font-weight:700; height:38px; text-decoration:none; display:inline-flex; align-items:center; gap:8px;"><span class="dashicons dashicons-edit"></span>  </a>
                    ...
                    <div style="position: relative; display: inline-block;">
                        ...
                        <div id="eess-absence-dropdown" style="display: none; position: absolute; left: 0; top: 110%; background: #fff; border: 1px solid #cbd5e1; border-radius: 8px; width: 220px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 99999; padding: 5px 0; text-align: right;">
                            <a href="javascript:void(0)" onclick="printAbsenceReport(\'daily\'); document.getElementById(\'eess-absence-dropdown\').style.display=\'none\';" style="display: block; padding: 10px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9; font-weight: 700;">  </a>
                            <a href="javascript:void(0)" onclick="printAbsenceReport(\'term\'); document.getElementById(\'eess-absence-dropdown\').style.display=\'none\';" style="display: block; padding: 10px 15px; color: #334155; font-size: 12px; text-decoration: none; font-weight: 700;">   (Training Group)</a>
                        </div>
                    </div>
            ...
            if ($active_tab === 'employee-profile') {
                $header_map['employee-profile']['button'] = '<button type="button" onclick="eessOpenProfileEditModal()" class="sm-btn" style="background: #000; border: 1px solid #000; color: #fff; border-radius: 8px; font-weight: 700; height: 38px; display: inline-flex; align-items: center; gap: 8px; cursor:pointer;"><span class="dashicons dashicons-edit"></span> Edit  </button>';
            }
            if ($active_tab === 'hr-evaluation') {
                $header_map['hr-evaluation']['button'] = '<button onclick="jQuery(\'#eess-new-eval-container\').slideToggle();" class="sm-btn" style="background: #000; border: 1px solid #000; color: #fff; border-radius: 8px; font-weight: 700; height: 38px; display: inline-flex; align-items: center; gap: 8px; cursor:pointer;"><span class="dashicons dashicons-plus-alt"></span>   </button>';
            }
                    ...
                    $btn_html .= '
                        ...
                        <div style="position: relative; display: inline-block;">
                            ...
                            <div id="eess-prep-reports-dropdown" style="display: none; position: absolute; left: 0; top: 110%; background: #fff; border: 1px solid #cbd5e1; border-radius: 8px; width: 250px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 99999; padding: 5px 0; text-align: right;">
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'submitted\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">   </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'not_submitted\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">❌   / </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'by_institution\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">   Organization </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'by_department\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">   </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'by_subject\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">   Sports Activities</a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'periodical\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">   (//Monthly)</a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'ranking\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">    </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'compliance\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;"> Intermediate No  </a>
                                <a href="javascript:void(0)" onclick="eessShowPrepReport(\'late_stats\')" style="display: block; padding: 8px 15px; color: #334155; font-size: 12px; text-decoration: none; border-bottom: 1px solid #f1f5f9;">⏱️    </a>
                                <a href="javascript:void(0)" onclick="eessExportPrepReport()" style="display: block; padding: 8px 15px; color: #0d9488; font-size: 12px; font-weight: bold; text-decoration: none;"> Export   (Excel/CSV)</a>
                            </div>
                        </div>
                ...
                if ($is_clinic_staff) {
                    ...
                    <div style="position: relative; display: inline-block;">
                        ...
                        <div id="eess-clinic-reports-dropdown" style="display: none; position: absolute; left: 0; top: 110%; background: #fff; border: 1px solid #cbd5e1; border-radius: 8px; width: 180px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); z-index: 99999; padding: 5px 0; text-align: right;">
                            <a href="' . admin_url('admin-ajax.php?action=sm_get_clinic_reports&report_type=day&nonce='.$c_nonce) . '" class="sm-dropdown-item"> </a>
                            <a href="' . admin_url('admin-ajax.php?action=sm_get_clinic_reports&report_type=week&nonce='.$c_nonce) . '" class="sm-dropdown-item"> </a>
                            <a href="' . admin_url('admin-ajax.php?action=sm_get_clinic_reports&report_type=month&nonce='.$c_nonce) . '" class="sm-dropdown-item"> </a>
                            <a href="' . admin_url('admin-ajax.php?action=sm_get_clinic_reports&report_type=term&nonce='.$c_nonce) . '" class="sm-dropdown-item"> Training Group</a>
                            <a href="' . admin_url('admin-ajax.php?action=sm_get_clinic_reports&report_type=year&nonce='.$c_nonce) . '" class="sm-dropdown-item"> </a>
                        </div>
                    </div>
                    ';
                }
            ...
            switch ($active_tab) {
                ...
                case 'students':
                    if ($is_admin || current_user_can('_No')) {
                ...
                case 'payments':
                    if (file_exists(SM_PLUGIN_DIR . 'templates/admin-payments.php')) {
                        include SM_PLUGIN_DIR . 'templates/admin-payments.php';
                    }
                    break;
                ...
                case 'stats':
                    if ($is_admin || current_user_can('_') || $is_parent) {
                ...
                case 'teachers':
                    if ($is_admin || current_user_can('_') || current_user_can('_Users')) {
                ...
                case 'global-settings':
                    if ($is_admin || current_user_can('_')) {
                        ?>
                        <div class="sm-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; overflow-x: auto; white-space: nowrap; padding-bottom: 10px;">
                            <button class="sm-tab-btn sm-active" onclick="smOpenInternalTab('school-settings', this)"></button>
                            <button class="sm-tab-btn" onclick="smOpenInternalTab('sidebar-settings', this)"> </button>
                            <button class="sm-tab-btn" onclick="smOpenInternalTab('backup-settings', this)">  No</button>
                            <?php if ($is_admin): ?>
                                <button class="sm-tab-btn" onclick="smOpenInternalTab('announcements-settings', this)">  </button>
                                <button class="sm-tab-btn" onclick="smOpenInternalTab('activity-logs', this)"> </button>
                            <?php endif; ?>
                        </div>
                        <div id="school-settings" class="sm-internal-tab">
                            <form method="post">
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                                    <div class="sm-form-group" style="grid-column: span 2;"><label class="sm-label"> Academy :</label><input type="text" name="school_name" value="<?php echo esc_attr($school['school_name']); ?>" class="sm-input"></div>
                                    ...
                                    <div class="sm-form-group" style="grid-column: span 2;">
                                        <label class="sm-label"> Academy :</label>
                                        <div style="display:flex; gap:10px;">
                                            <input type="text" name="school_logo" id="sm_school_logo_url" value="<?php echo esc_attr($school['school_logo']); ?>" class="sm-input">
                                            <button type="button" onclick="smOpenMediaUploader('sm_school_logo_url')" class="sm-btn" style="width:auto; font-size:12px; background:var(--sm-secondary-color);">/</button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="sm-form-group" style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; margin-top: 25px;">
                                    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 10px;">
                                        ...
                                        <h4 style="margin: 0; color: var(--sm-dark-color); font-weight: 800; font-size: 1.1em;"> : Sportedia Sports Management System</h4>
                                    </div>
                                    <p style="margin: 5px 0; font-size: 13px; color: var(--sm-text-gray); line-height: 1.6;">
                                               <strong>    (Educational Electronic Systems Services - EESS)</strong>            .
                                    </p>
                                    <div style="margin-top: 15px; display: flex; gap: 20px; font-size: 13px; font-weight: 700;">
                                        <div>   : <a href="https://eess.online" target="_blank" style="color: var(--sm-primary-color); text-decoration: none;">eess.online</a></div>
                                        <div>   : <a href="mailto:info@eess.online" style="color: var(--sm-primary-color); text-decoration: none;">info@eess.online</a></div>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div id="sidebar-settings" class="sm-internal-tab" style="display:none;">
                            <form method="post">
                                ...
                                <p style="font-size:12px; color:#666; margin-bottom:20px;">         .</p>
                                ...
                                <button type="submit" name="sm_save_sidebar_visibility" class="sm-btn" style="width:auto; margin-top: 20px;">Save  </button>
                            </form>
                        </div>
                            ...
                            <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:30px;">
                                ...
                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:30px;">
                                    <div style="background:white; padding:15px; border-radius:8px; border:1px solid #eee;">
                                        <div style="font-size:12px; color:#718096;"> Export :</div>
                                        <div style="font-weight:700; color:var(--sm-primary-color);"><?php echo $backup_info['export']; ?></div>
                                    </div>
                                    <div style="background:white; padding:15px; border-radius:8px; border:1px solid #eee;">
                                        <div style="font-size:12px; color:#718096;"> Import :</div>
                                        <div style="font-weight:700; color:var(--sm-secondary-color);"><?php echo $backup_info['import']; ?></div>
                                    </div>
                                </div>
                                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:20px;">
                                    <div style="background:white; padding:20px; border-radius:8px; border:1px solid #eee;">
                                        <h5 style="margin-top:0;">Export  </h5>
                                        <p style="font-size:12px; color:#666; margin-bottom:15px;"> Upload     No   JSON.</p>
                                        <div style="display:flex; gap:10px;">
                                            <form method="post">
                                                <?php wp_nonce_field('sm_admin_action', 'sm_admin_nonce'); ?>
                                                <button type="submit" name="sm_download_backup" class="sm-btn" style="background:#27ae60; width:auto;">Export  (JSON)</button>
                                            </form>
                                            <form method="get" action="<?php echo admin_url('admin-ajax.php'); ?>">
                                                ...
                                                <button type="submit" class="sm-btn" style="background:#111F35; width:auto;"> No  (CSV)</button>
                                            </form>
                                        </div>
                                    </div>
                                    <div style="background:white; padding:20px; border-radius:8px; border:1px solid #eee;">
                                        <h5 style="margin-top:0;">Export No No </h5>
                                        <p style="font-size:12px; color:#666; margin-bottom:15px;">Export   No     .</p>
                                        <form method="get" action="<?php echo admin_url('admin-ajax.php'); ?>" target="_blank">
                                            ...
                                            <div class="sm-form-group">
                                                <input type="text" name="student_code" class="sm-input" placeholder="  No (: ST00001)" required style="font-size:11px;">
                                            </div>
                                            <button type="submit" class="sm-btn" style="background:#3182ce; width:auto; font-size:11px;">Export No No</button>
                                        </form>
                                    </div>
                                    <div style="background:white; padding:20px; border-radius:8px; border:1px solid #eee;">
                                        <h5 style="margin-top:0;">Import </h5>
                                        <p style="font-size:12px; color:#e53e3e; margin-bottom:15px;">:  Import      .</p>
                                        <form method="post" enctype="multipart/form-data">
                                            <?php wp_nonce_field('sm_admin_action', 'sm_admin_nonce'); ?>
                                            <input type="file" name="backup_file" required style="margin-bottom:10px; font-size:11px;">
                                            <button type="submit" name="sm_restore_backup" class="sm-btn" style="background:#2980b9; width:auto;"> Import</button>
                                        </form>
                                    </div>
                                    <div style="background:white; padding:20px; border-radius:8px; border:1px solid #eee;">
                                        <h5 style="margin-top:0;">  </h5>
                                        <p style="font-size:12px; color:#666; margin-bottom:15px;">       :</p>
                                        <div style="display:flex; flex-wrap:wrap; gap:10px;">
                                            <button onclick="smBulkDelete('students')" class="sm-btn sm-btn-outline" style="font-size:11px; color:#e53e3e; border-color:#feb2b2;"> No</button>
                                            <button onclick="smBulkDelete('teachers')" class="sm-btn sm-btn-outline" style="font-size:11px; color:#e53e3e; border-color:#feb2b2;"> </button>
                                            <button onclick="smBulkDelete('parents')" class="sm-btn sm-btn-outline" style="font-size:11px; color:#e53e3e; border-color:#feb2b2;">  Mother </button>
                                            <button onclick="smBulkDelete('records')" class="sm-btn sm-btn-outline" style="font-size:11px; color:#e53e3e; border-color:#feb2b2;"> </button>
                                        </div>
                                    </div>
                                    <div style="background:#fff5f5; padding:20px; border-radius:8px; border:2px dashed #feb2b2;">
                                        <h5 style="margin-top:0; color:#c53030;">  (Reset )</h5>
                                        <p style="font-size:12px; color:#666; margin-bottom:15px;">    ****        Settings Users No.</p>
                                        <button onclick="smInitializeSystem()" class="sm-btn" style="background:#c53030; width:auto;">  </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="activity-logs" class="sm-internal-tab" style="display:none;">
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:24px; box-shadow: 0 4px 18px rgba(0,0,0,0.02);">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:15px;">
                                    <div>
                                        <h4 style="margin:0; font-size:16px; font-weight:800; color:#0f172a;">    (Activity Log)</h4>
                                        <div style="font-size:12px; color:#64748b; margin-top:3px;"> No   100      .</div>
                                    </div>
                                    <div style="display:flex; gap:10px; align-items:center;">
                                        <button type="button" onclick="smDeleteAllLogs()" class="sm-btn" style="background:#fee2e2; color:#b91c1c !important; border:1px solid #fca5a5; height:36px; padding:0 16px; font-size:12px; border-radius:9999px; font-weight:800; cursor:pointer;" title="  No ">
                                ...
                                <div style="margin-bottom: 20px;">
                                    <input type="text" id="eess-activity-log-search" onkeyup="eessFilterActivityLog()" placeholder="Search Username      ..." class="sm-input" style="height: 40px; border-radius: 10px; font-size: 12.5px; border: 1px solid #cbd5e1; padding: 0 14px; width: 100%; box-sizing: border-box;">
                                </div>
                                ...
                                <div class="sm-table-container">
                                    <table class="sm-table" id="eess-activity-log-table">
                                        ...
                                        <tbody>
                                            ...
                                            if (empty($all_logs)): ?>
                                                <tr>
                                                    <td colspan="6" style="text-align:center; color:#94a3b8; padding:30px; font-weight:700;">No     .</td>
                                                </tr>
                                            <?php else:
                                                foreach ($all_logs as $log):
                                                    $can_rollback = strpos($log->details, 'ROLLBACK_DATA:') === 0;
                                                    $details_display = $can_rollback ? '  ' : esc_html($log->details);
                                                ...
                                                <tr class="eess-log-row" data-search="<?php echo esc_attr(strtolower($log->display_name . ' ' . $log->employee_number . ' ' . $log->role_label . ' ' . $log->action . ' ' . $log->details)); ?>">
                                                    ...
                                                    <td style="text-align:center;">
                                                        <?php if ($is_mobile_source): ?>
                                                            <span style="font-size:10px; font-weight:800; background:#fef3c7; color:#b45309; border:1px solid #fde68a; padding:2px 8px; border-radius:9999px;" title="    "> </span>
                                                        <?php else: ?>
                                                            <span style="font-size:10px; font-weight:800; background:#e0f2fe; color:#0369a1; border:1px solid #bae6fd; padding:2px 8px; border-radius:9999px;" title="    "> </span>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td style="text-align:center;">
                                                        <div style="display:flex; gap:4px; justify-content:center;">
                                                            <?php if ($can_rollback): ?>
                                                                <button type="button" onclick="smRollbackLog(<?php echo $log->id; ?>)" class="sm-btn" style="padding:0 !important; width:28px !important; min-width:28px !important; height:28px !important; background:#dcfce7 !important; border:1px solid #86efac !important; color:#15803d !important; border-radius:6px !important; cursor:pointer; display:inline-flex !alignment; align-items:center; justify-content:center;" title="  ">
                                                                    <span class="dashicons dashicons-undo" style="font-size:14px; margin:0;"></span>
                                                                </button>
                                                            <?php endif; ?>
                                                            <button type="button" onclick="smDeleteLog(<?php echo $log->id; ?>)" class="sm-btn" style="padding:0 !important; width:28px !important; min-width:28px !important; height:28px !important; background:#fee2e2 !important; border:1px solid #fca5a5 !important; color:#b91c1c !important; border-radius:6px !important; cursor:pointer; display:inline-flex !alignment; align-items:center; justify-content:center;" title="Delete  ">
                                                                <span class="dashicons dashicons-trash" style="font-size:14px; margin:0;"></span>
                                                            </button>
                                                        </div>
                                                    </td>
            ...
            if (label && label.length > 0) {
                ...
                if (text && (!placeholder || placeholder === text)) {
                    if (input.is('select')) {
                        ...
                        if (firstOpt && (firstOpt.val() === '' || firstOpt.text().includes('--') || firstOpt.text().includes(''))) {
                            firstOpt.text(text);
                        }
                    } else {
...
<div id="sm-global-confirm-modal" class="sm-modal-overlay" style="display: none; position: fixed; inset: 0; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); z-index: 99999; align-items: center; justify-content: center;" dir="rtl">
    <div style="background: #ffffff; border-radius: 16px; max-width: 440px; width: 90%; padding: 24px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.2); font-family: 'Cairo', sans-serif; text-align: center; border: 1px solid #e2e8f0;">
        ...
        <h3 id="sm-confirm-title" style="margin: 0 0 8px 0; font-size: 1.25rem; font-weight: 800; color: #0f172a;">Confirm </h3>
        <p id="sm-confirm-message" style="margin: 0 0 24px 0; font-size: 0.9rem; color: #64748b; line-height: 1.6;">      </p>
...
window.smConfirmAction = function(options) {
    return new Promise(function(resolve) {
        ...
        if (titleEl) titleEl.innerText = options.title || 'Confirm ';
    ...
    <div style="background: #ffffff; border-radius: 20px; max-width: 520px; width: 100%; border: 1px solid #cbd5e1; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.3); overflow: hidden;">
        <div style="background: #0f172a; color: #ffffff; padding: 18px 24px; display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <h3 style="margin: 0; font-size: 16px; font-weight: 800; color: #ffffff;">    </h3>
            </div>
        ...
        <form id="eess-copy-record-form" onsubmit="eessExecuteCopyRecordSubmit(event)" style="padding: 24px;">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 16px; margin-bottom: 18px;">
                <div style="font-size: 11.5px; font-weight: 700; color: #64748b;">  :</div>
                <div id="copy_record_title_preview" style="font-size: 14px; font-weight: 800; color: #0f172a; margin-top: 2px;">-</div>
            </div>

            <div style="margin-bottom: 18px;">
                <label style="font-size: 12.5px; font-weight: 800; color: #334155; margin-bottom: 6px; display: block;">  /   <span style="color:#ef4444;">*</span></label>
                <?php
            ...
            <div id="copy_target_user_details" style="display: none; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 12px 16px; margin-bottom: 20px;">
                <div style="font-size: 11.5px; font-weight: 700; color: #166534; margin-bottom: 4px;">Confirm  :</div>
                <div style="font-size: 12.5px; color: #15803d; font-weight: 700;">
                    : <span id="copy_target_email_text">-</span> | : <span id="copy_target_role_text">-</span>
                </div>
            </div>

            <div style="display: flex; gap: 12px; justify-content: flex-end;">
                <button type="submit" id="copy_submit_btn" class="sm-btn" style="background: #000000; color: #ffffff !important; height: 40px; padding: 0 22px; font-weight: 800; border-radius: 9999px !important; border: none; cursor: pointer;">Confirm  </button>
...
window.eessOpenCopyRecordModal = function(recordType, recordId, recordTitle) {
    ...
    document.getElementById('copy_record_title_preview').innerText = recordTitle || '  ';
...
<div id="eess-support-capsule-modal" style="display: none; position: fixed; inset: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.75); backdrop-filter: blur(5px); z-index: 999999; justify-content: center; align-items: center; padding: 20px; box-sizing: border-box; font-family: 'Cairo', sans-serif; direction: rtl;">
    <div style="background: #ffffff; border-radius: 20px; max-width: 640px; width: 100%; border: 1px solid #e2e8f0; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); overflow: hidden; display: flex; flex-direction: column; padding: 28px; position: relative;">
        ...
        <div style="text-align: center; margin-bottom: 20px;">
            ...
            <h3 style="margin: 0 0 4px 0; font-size: 18px; font-weight: 800; color: #0f172a;">  </h3>
            <p style="margin: 0; font-size: 12.5px; color: #64748b; font-weight: 600;">         </p>
        </div>
        ...
        <div style="box-sizing: border-box;">
            ...
            <div id="capsule-menu-view">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
                    ...
                    <button type="button" onclick="eessSelectCapsuleOption('suggestion')" style="background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 16px; padding: 20px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; cursor: pointer; transition: all 0.2s ease; aspect-ratio: 1 / 1;" onmouseover="this.style.borderColor='#881337'; this.style.transform='translateY(-3px)';" onmouseout="this.style.borderColor='#cbd5e1'; this.style.transform='translateY(0)';">
                        ...
                        <div style="font-weight: 800; font-size: 14.5px; color: #0f172a; margin-bottom: 6px;"> </div>
                        <div style="font-size: 11px; color: #64748b; line-height: 1.4;">   </div>
                    </button>
                    ...
                    <button type="button" onclick="eessSelectCapsuleOption('technical_issue')" style="background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 16px; padding: 20px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; cursor: pointer; transition: all 0.2s ease; aspect-ratio: 1 / 1;" onmouseover="this.style.borderColor='#881337'; this.style.transform='translateY(-3px)';" onmouseout="this.style.borderColor='#cbd5e1'; this.style.transform='translateY(0)';">
                        ...
                        <div style="font-weight: 800; font-size: 14.5px; color: #0f172a; margin-bottom: 6px;"> </div>
                        <div style="font-size: 11px; color: #64748b; line-height: 1.4;">No     </div>
                    </button>
                    ...
                    <button type="button" onclick="eessSelectCapsuleOption('rating')" style="background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 16px; padding: 20px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; cursor: pointer; transition: all 0.2s ease; aspect-ratio: 1 / 1;" onmouseover="this.style.borderColor='#881337'; this.style.transform='translateY(-3px)';" onmouseout="this.style.borderColor='#cbd5e1'; this.style.transform='translateY(0)';">
                        ...
                        <div style="font-weight: 800; font-size: 14.5px; color: #0f172a; margin-bottom: 6px;"></div>
                        <div style="font-size: 11px; color: #64748b; line-height: 1.4;">  Send  </div>
                    </button>
                </div>
            </div>
            ...
            <div id="capsule-suggestion-view" style="display: none;">
                <h4 style="margin: 0 0 15px 0; font-size: 14px; font-weight: 800; color: #0f172a; border-bottom: 2px solid #f1f5f9; padding-bottom: 8px;">  </h4>
                <form id="eess_capsule_sug_form" onsubmit="eessSubmitCapsuleSuggestion(event)">
                    <div style="margin-bottom: 12px;">
                        ...
                        <input type="text" id="sug_title" required placeholder="  " style="width: 100%; height: 40px; border: 1px solid #cbd5e1; border-radius: 8px; padding: 0 10px; font-size: 13px; box-sizing: border-box;">
                    </div>

                    <div style="margin-bottom: 15px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                            <label style="font-size: 12px; font-weight: 700; color: #334155;">  <span style="color:#ef4444;">*</span></label>
                            <span id="sug_counter" style="font-size: 11px; font-weight: 700; color: #64748b;">0 / 1000</span>
                        </div>
                        <textarea id="sug_details" required maxlength="1000" rows="4" oninput="document.getElementById('sug_counter').innerText = this.value.length + ' / 1000'" placeholder="        ..." style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 10px; font-size: 13px; box-sizing: border-box;"></textarea>
                    </div>

                    <div style="display: flex; gap: 10px; justify-content: space-between;">
                        <button type="button" onclick="eessBackToCapsuleMenu()" class="sm-btn sm-btn-outline" style="height: 38px; font-size: 12px;">←  </button>
                        <button type="submit" id="btn_sug_submit" class="sm-btn" style="background: #2563eb; height: 38px; font-size: 12px; padding: 0 20px;">Send </button>
                    </div>
                </form>
            </div>
            ...
            <div id="capsule-tech-view" style="display: none;">
                ...
                <div id="tech-step-1" style="display: block;">
                    <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 12px; padding: 15px; margin-bottom: 15px;">
                        <h4 style="margin: 0 0 6px 0; color: #991b1b; font-size: 14px; font-weight: 800;">No   </h4>
                        <p style="margin: 0; font-size: 12px; color: #b91c1c; line-height: 1.6;">   No         .    No    .</p>
                    </div>
                    <div style="display: flex; gap: 10px; justify-content: space-between;">
                        <button type="button" onclick="eessBackToCapsuleMenu()" class="sm-btn sm-btn-outline" style="height: 38px; font-size: 12px;">←  </button>
                        <button type="button" onclick="document.getElementById('tech-step-1').style.display='none'; document.getElementById('tech-step-2').style.display='block';" class="sm-btn" style="background: #2563eb; height: 38px; font-size: 12px; padding: 0 20px;">  Next →</button>
                    </div>
                </div>
                ...
                <div id="tech-step-2" style="display: none;">
                    <form id="eess_capsule_tech_form" onsubmit="eessSubmitCapsuleTechnicalIssue(event)">
                        <div style="margin-bottom: 12px;">
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #334155; margin-bottom: 4px;">  <span style="color:#ef4444;">*</span></label>
                            <input type="text" id="tech_title" required placeholder="  " style="width: 100%; height: 40px; border: 1px solid #cbd5e1; border-radius: 8px; padding: 0 10px; font-size: 13px; box-sizing: border-box;">
                        </div>

                        <div style="margin-bottom: 12px;">
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #334155; margin-bottom: 4px;">  <span style="color:#ef4444;">*</span></label>
                            <textarea id="tech_details" required rows="3" placeholder="     ..." style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 10px; font-size: 13px; box-sizing: border-box;"></textarea>
                        </div>

                        <div style="margin-bottom: 15px;">
                            <label style="display: block; font-size: 12px; font-weight: 700; color: #334155; margin-bottom: 4px;">   (Screenshot)</label>
                        ...
                        <div style="display: flex; gap: 10px; justify-content: space-between;">
                            ...
                            <button type="submit" id="btn_tech_submit" class="sm-btn" style="background: #dc2626; height: 38px; font-size: 12px; padding: 0 20px;">Send </button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div id="capsule-rating-view" style="display: none;">
                ...
                <form id="eess_capsule_rating_form" onsubmit="eessSubmitCapsuleRating(event)">
                    ...
                    <div style="text-align: center; margin-bottom: 15px;">
                        <label style="display: block; font-size: 12px; font-weight: 700; color: #334155; margin-bottom: 8px;">  :</label>
                    ...
                    <div style="margin-bottom: 15px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                            <label style="font-size: 12px; font-weight: 700; color: #334155;">   </label>
                            <span id="rating_counter" style="font-size: 11px; font-weight: 700; color: #64748b;">0 / 250</span>
                        </div>
                        <textarea id="rating_comment" maxlength="250" rows="3" oninput="document.getElementById('rating_counter').innerText = this.value.length + ' / 250'" placeholder="      ..." style="width: 100%; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 10px; font-size: 13px; box-sizing: border-box;"></textarea>
                    </div>

                    <div style="display: flex; gap: 10px; justify-content: space-between;">
                        <button type="button" onclick="eessBackToCapsuleMenu()" class="sm-btn sm-btn-outline" style="height: 38px; font-size: 12px;">←  </button>
                        <button type="submit" id="btn_rating_submit" class="sm-btn" style="background: #16a34a; height: 38px; font-size: 12px; padding: 0 20px;">Send </button>
                    </div>
                </form>
            </div>

...
window.eessGoToStep = function(step) {
    ...
    if (eessCurrentStep === 5) {
        ...
        if (usernameDisplay) {
            usernameDisplay.value = empIdVal ? empIdVal : '     ';
        }
    }
};
...
window.eessRenderStepSummary = function() {
    ...
    var nat = document.getElementById('u_nationality').value || ' ';
    var dob = document.getElementById('u_dob').value || ' ';
    var genderSel = document.getElementById('u_gender');
    var genderTxt = genderSel.options[genderSel.selectedIndex] ? genderSel.options[genderSel.selectedIndex].text : ' ';
    ...
    var emirate = document.getElementById('u_emirate').value || ' ';
    var address = ' ';
    var bldg = ' ';
    ...
    if (roleVal === 'administrator') {
        ...
        if (document.getElementById('rev_u_grades')) document.getElementById('rev_u_grades').innerText = (checkedGrades.length > 0 ? checkedGrades.join(' ') : ' ') + ' — : ' + sectionsTxt;
    }
};
...
window.eessCheckUniqueness = function(field) {
    ...
    .then(function(res) {
        if (res.success && res.data.exists) {
            ...
            if (errEl) {
                errEl.innerText = res.data.message || '    .';

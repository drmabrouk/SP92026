...
class SM_Public {
                ...
                <div style="background: #ffffff; border-radius: 16px; padding: 16px; border: 1px solid #cbd5e1; margin-bottom: 16px;">
                    ...
                    <?php if (!empty($pending_teachers)): ?>
                        <div style="background: #f8fafc; border-radius: 8px; padding: 10px; border: 1px solid #e2e8f0; font-size: 11px; color: #475569;">
                            <strong style="color: #991b1b; display: block; margin-bottom: 4px;">قائمة المدربين المتبقين دون رفع تحضير:</strong>
    ...
    public function ajax_bulk_delete() {
        ...
        switch ($type) {
            ...
            case 'teachers':
                ...
                SM_Logger::log('مسح كافة المدربين', 'إجراء جماعي');
                break;
            case 'parents':
    ...
    public function handle_form_submission() {
        ...
        if (isset($_POST['sm_add_teacher']) && wp_verify_nonce($_POST['sm_nonce'], 'sm_teacher_action')) {
            if (current_user_can('إدارة_المدربين')) {
                $user_data = array(
                    'user_login' => sanitize_user($_POST['user_login']),
                    'user_email' => sanitize_email($_POST['user_email']),
                    'display_name' => sanitize_text_field($_POST['display_name']),
        ...
        if (isset($_POST['sm_import_teachers_csv']) && wp_verify_nonce($_POST['sm_admin_nonce'], 'sm_admin_action')) {
            if (current_user_can('إدارة_المدربين') && !empty($_FILES['csv_file']['tmp_name'])) {
                $handle = fopen($_FILES['csv_file']['tmp_name'], "r");
                $header = fgetcsv($handle); // skip header
                $count = 0;
                while (($data = fgetcsv($handle)) !== FALSE) {
    ...
    public function ajax_export_students_pdf() {
        ...
        $inst_name = 'جميع الأكاديميات الرياضية والمؤسسات';
        if ($school_id > 0) {
    ...
    public function ajax_sm_mark_teacher_contacted() {
        ...
        if (!$teacher_id) wp_send_json_error('معرف المدرب غير صحيح.');
        ...
        SM_Logger::log('التواصل عبر واتساب', "تم توثيق التواصل عبر واتساب مع المدرب ID: $teacher_id للسجل: $record_type ($record_id)");

        wp_send_json_success(array('message' => 'تم توثيق التواصل مع المدرب بنجاح.'));
    }
    ...
    public function ajax_sm_assign_term_plan() {
        ...
        if (!$target_uid) wp_send_json_error('يرجى اختيار المدرب المستهدف.');
    ...
    public function ajax_sm_assign_lesson_prep() {
        ...
        if (!$target_uid || empty($title)) wp_send_json_error('يرجى اختيار المدرب وتحديد عنوان الدرس.');
    ...
    public function ajax_sm_print() {
        ...
        if ($print_type === 'lesson_prep') {
            ...
            <head>
                <meta charset="UTF-8">
                <title>تقرير الكادر غير الملتزم بتسليم تقديم تقارير أعضاء هيئة التدريب والكادر</title>
                <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800;900&display=swap" rel="stylesheet">
                <style>
            ...
            $school_name = !empty($school_info['name']) ? $school_info['name'] : 'نظام EESS الموحد لإدارة الأكاديميات الرياضية';
            ...
            <body>
                ...
                <div class="summary-grid">
                    <div class="summary-card">
                        <div style="font-size: 10px; color: #64748b; font-weight: 700;">إجمالي المدربين</div>
            ...
            $report_title = 'تقرير الكادر غير الملتزم بتسليم تقديم تقارير أعضاء هيئة التدريب والكادر — ' . $target_school;
    ...
    public function ajax_bulk_download_lesson_preps() {
        ...
        foreach ($records as $rec) {
            ...
            $zip_folder = sanitize_file_name($rec->teacher_name ?: 'المدرب');
